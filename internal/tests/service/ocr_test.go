...
var Images = []string{
	"https://minio-api.go-cinch.top/ocr/2024-11-13/6.jpg",
...
func TestOssService_BatchOcr(t *testing.T) {
	...
	go func() {
		_ = callOcr(context.Background(), lo.Shuffle(Images[:1]))
	}()
	ch := make(chan struct{})
	<-ch
}
...
func callOcr(ctx context.Context, images []string) (err error) {
	...
	req.Header.Add("Authorization", "Bearer eyJhbGciOiJIUzUxMiIsInR5cCI6IkpXVCJ9.eyJjb2RlIjoiODlIRUsyOFkiLCJleHAiOjE3MzE1NTIyNzMsInBsYXRmb3JtIjoicGMifQ.1wZKXfRAhJ01eKWmx9NfRSPmaP18y2WiN_33qfwla8WB1iQD8mGh-UdkokJ_k3CLxtUlMU3686DqCT6_duWgDA")
