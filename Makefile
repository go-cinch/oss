...
GRPC_PORT:=$(shell grep -A 5 'grpc:' configs/config.yml | grep -v '\#' | grep 'addr:' | awk -F':' '{print $$3}' | tr -d ' ')
...
init:
	...
	go install github.com/fullstorydev/grpcui/cmd/grpcui@v1.4.1
...
# debug-grpc
debug-grpc:
	@grpcui -plaintext -import-path=./api/oss-proto -import-path=./third_party -proto=oss.proto "127.0.0.1:$(GRPC_PORT)"
